...
const SingleProduct = ({ data, variations }) => {
  ...
  useEffect(() => {
    ...
    if (gallery_video_url_1) {
      newImages.splice(1, 0, {
        id: 208234,
        src: gallery_thumbnail_1?.url,
        video: true,
        alt: "",
        link: gallery_video_url_1,
      });
    }

    if (gallery_thumbnail_2) {
      newImages.splice(2, 0, {
        id: 208235,
        src: gallery_thumbnail_2?.url,
        video: true,
        alt: "",
        link: gallery_video_url_2,
      });
    }
    ...
    return (
      <div className="uppercase md:mb-3 md:mx-0 -mx-5 md:bg-white bg-black -mb-1 md:pt-0 pt-5 md:px-0 px-5 md:pb-0 pb-5">
        <div className="font-bold text-sm text-[#999999] flex flex-wrap">
          ...
          {breadcrumb?.map((item, i) => {
            ...
            return (
              <span
                key={i}
                className={`${
                  i === breadcrumb?.length - 1 ? "block" : "hidden"
                }`}
              >
                <Link href={finalUrl} className="text-[#999999]">
                  {"... /"} {item?.name}
                </Link>
          ...
          <span className="lg:text-black text-white">{data?.name}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div>
      ...
      <div
        ref={containerRef}
        id="product-content"
        className={`global-padding md:pt-4 pt-0 max-w-[1920px] mx-auto w-full relative ${
          isLanding ? "hidden" : "block"
        }`}
      >
        <BreadCums />
        <div className="flex items-start md:flex-row flex-col justify-between gap-10">
          <div className="md:w-[60%] w-full">
            <div className="md:grid lg:grid-cols-2 md:grid-cols-1 gap-2.5 relative hidden">
              {images?.slice(0, sliceLength)?.map((singleImage, i) => {
                return (
                  <div
                    ...
                    className="rounded-sm overflow-hidden bg-black relative cursor-pointer"
                    key={i}
                  >
                    <Image
                      src={singleImage?.src || default_image}
                      width={649}
                      height={649}
                    ...
                    {singleImage?.video && (
                      <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
                        ...
                        >
                          ...
                          ></rect>
                          <path
                            d="M37 26.2679C38.3333 27.0377 38.3333 28.9623 37 29.7321L25 36.6603C23.6667 37.4301 22 36.4678 22 34.9282L22 21.0718C22 19.5322 23.6667 18.5699 25 19.3397L37 26.2679Z"
                            fill="white"
                          ></path>
                        </svg>
                      </span>
              ...
              {sliceLength === 4 && (
                <button
                  className="px-4 py-2 rounded-[20px] bg-white border-[#ccc] border w-fit text-base leading-6 font-semibold absolute left-1/2 -translate-x-1/2 -bottom-5 cursor-pointer"
                  onClick={() => setLength(images?.length)}
                >
                  View all
                </button>
              )}
              ...
              <div className="w-full h-full mx-auto flex flex-col items-center justify-center relative">
                              ...
                              {activeIndex === i ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center bg-black rounded-sm">
                                  <Image
                                    ...
                                    className="w-full h-full rounded-sm object-contain aspect-[1]"
                                    width={649}
                                    height={649}
                ...
                <div className="relative px-3 py-[10px] w-full z-50 backdrop-blur-[4px] border border-gray-200 rounded-[4px] items-center justify-center bg-white/20 gap-2 md:flex hidden max-w-fit mx-auto">
                  {images?.map((singleImage, index) => {
                    const isActive =
                      activeIndex === index || default_slide === index;
                    return (
                      <div
                        onClick={() => swiperRef.current?.slideTo(index)}
                        key={singleImage?.id}
                        className={`overflow-hidden rounded-[4px] relative cursor-pointer transition-opacity duration-300 ${
                      ...
                      >
                        <Image
                          ...
                          height={54}
                          alt=""
                          className="w-[54px] h-[54px] aspect-[1]"
                        />
                        {singleImage?.video && (
                          <span
      ...
      {!!LandingPage && (
        <div id="product-landing-section">
          <LandingPage data={data} setIsLanding={setIsLanding} />
        </div>
      )}
    </div>
  );
